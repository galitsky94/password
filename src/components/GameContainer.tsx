...
const GameContainer = () => {
  ...
  const [curve, setCurve] = useState(50); // New curve slider
  ...
  const calculateTrajectory = () => {
    ...
    // Updated range formula: increases with power, scales with gravity
    const powerRatio = power / 100;
    const powerScaleFactor = Math.pow(powerRatio, 2);
    const curveRatio = curve / 100;
    const baseRange = 100 + powerScaleFactor * 2000 * curveRatio; // curve scales distance

    // Gravity reduces effective range (up to 4x shorter at gravity 100)
    const gravityDistanceFactor = 1 / (1 + (gravity / 100) * 3);
    const range = baseRange * gravityDistanceFactor;

    // Height uses curve slider (curveness) - use the previously declared curveRatio
    const maxHeight = 5 + Math.pow(curveRatio, 2) * 295;
  ...
  const handleLaunch = () => {
    ...
    animateShot(fullTrajectory, 2000); // Animate over 2 seconds (changed from 1000ms)
  };
    ...
    <div className="max-w-5xl w-full bg-white rounded-lg shadow-lg p-6">
      ...
      <div className="space-y-4">
        ...
        <div className="grid grid-cols-3 gap-6">
          ...
          <div>
            <div className="flex justify-between">
              <label htmlFor="curve" className="text-gray-700 font-medium">
                Curve:
              </label>
